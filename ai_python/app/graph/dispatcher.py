...
def dispatch(tool_name: str, *, raw_require: str, upstream_data: dict[str, Any],
    ...
    result = _invoke_subgraph(tool_name, payload, llm=llm, deps=deps)
    log.debug("dispatch tool=%s valid=%s validation_error=%s",
              tool_name, result["valid"], result["validation_error"])
    return result
