...
def commit_inventory_draft(
    ...
    *,
    ...
    url = urljoin(inventory_drafts_url(settings).rstrip("/") + "/", f"{quote(str(draft_id))}/commit")
    ...
    try:
        ...
        raise RuntimeError("Spring inventory-drafts commit timeout") from exc
    except httpx.TransportError as exc:
        raise RuntimeError(f"Spring inventory-drafts commit transport error: {exc}") from exc
    if resp.status_code >= 400:
        detail = resp.text[:500] if resp.text else resp.status_code
        raise RuntimeError(f"Spring inventory-drafts commit HTTP {resp.status_code}: {detail}")
    data = resp.json()
    if not isinstance(data, dict):
        raise RuntimeError("Spring inventory-drafts commit response is not JSON object")
    inner = data.get("data")
