...
def commit_catalog_draft(
    ...
    *,
    ...
    url = urljoin(catalog_drafts_url(settings).rstrip("/") + "/", f"{quote(str(draft_id))}/commit")
    ...
    try:
        ...
        raise RuntimeError("Spring catalog-drafts commit timeout") from exc
    except httpx.TransportError as exc:
        raise RuntimeError(f"Spring catalog-drafts commit transport error: {exc}") from exc
    if resp.status_code >= 400:
        detail = resp.text[:500] if resp.text else resp.status_code
        raise RuntimeError(f"Spring catalog-drafts commit HTTP {resp.status_code}: {detail}")
    data = resp.json()
    if not isinstance(data, dict):
        raise RuntimeError("Spring catalog-drafts commit response is not JSON object")
    inner = data.get("data")
