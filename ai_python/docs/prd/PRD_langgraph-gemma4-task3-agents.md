...
- `ai_python/docs/prd/PRD_langgraph-gemma4-task1.md` — Option B: port `LlmClient` + registry role. **Mọi node Agent gọi qua `get_llm_client(role)`**, không tạo `ChatOpenAI` thủ công.
- `ai_python/docs/prd/PRD_langgraph-gemma4-task2.md` — Skeleton LangGraph + subgraph SQL + retry cap 3 + port `SqlExecutor` (Option C) + checkpointer factory. **Task 3 build tiếp lên skeleton, KHÔNG sửa topology graph.**

**Tham chiếu DESIGN:**

- `ai_python/TASKS/DESIGN/TASK_AGENTS_V1_DESIGN.md`
- `ai_python/TASKS/DESIGN/TASK_LANGGRAPH_GEMMA4_TRIEN_KHAI.md` (§6 `TASK-AG-*`, `SUM-*`, `REG-*`, `CTX-*`)
...
- Thêm **một trang** `ai_python/docs/intent_registry_howto.md`: checklist cách đăng ký intent mới + đồng bộ enum/Pydantic `IntentLabel`.
...
- [ ] **REG-01..03 registry hardening — unknown → general_chat + howto**
  - **Description:** Unknown intent → `general_chat` (D5); file `ai_python/docs/intent_registry_howto.md` checklist thêm intent.
