...
**Postman** (wrapper `_description` + `request` + `headers` + `body`): [`docs/backend/smart-erp/postman/`](../../../../docs/backend/smart-erp/postman/) — chuẩn **3 file** theo `TESTER_AGENT_INSTRUCTIONS.md`; Doc Sync giữ **cùng field** với `samples/*/.*.request.json` khi đổi contract.
