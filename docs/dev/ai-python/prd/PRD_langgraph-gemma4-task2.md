...
**Tham chiếu thiết kế:** `docs/ai-python/tasks/DESIGN/TASK_LANGGRAPH_GEMMA4_TRIEN_KHAI.md` (Task 2 — **chỉ** TASK-LG-01 … LG-14; TASK-AG-* thuộc task thiết kế Agent riêng).  
**Căn chỉnh Task 1:** `docs/ai-python/prd/PRD_langgraph-gemma4-task1.md` — nodes LangGraph gọi **`LlmClient` / registry role** (Option B đã chốt), không tạo `ChatOpenAI` rải rác.
