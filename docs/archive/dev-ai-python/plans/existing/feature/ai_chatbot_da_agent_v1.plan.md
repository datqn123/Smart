...
Plan này mô tả **chức năng v1** và **cách ánh xạ sang LangGraph** (state graph, conditional edges, subgraph, retry có giới hạn, checkpointer/streaming tuỳ chọn). Tham chiếu stack tổng quan: các tài liệu `docs/plans/existing/` / phân tích LangChain–MCP trong repo (nếu có).
...
- **Cổng Spring**: xác thực user, kiểm tra **được phép dùng AI**, rồi gọi [`ai_python`](../../../docs/ai-python/README.md) (FastAPI) kèm context an toàn.
