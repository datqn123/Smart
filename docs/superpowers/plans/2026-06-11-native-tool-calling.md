...
- [x] **Step 1: Viết test fail** — thêm vào cuối `tests/test_llm_client.py`:
...
- [x] **Step 3: Implement** — thêm vào `app/config/llm_client.py` (sau class `OpenAILLMClient.complete`, trong cùng class):
...
- [x] **Step 4: Run → pass**: `pytest tests/test_llm_client.py -v` → PASS (test cũ + mới).
- [x] **Step 5: Full suite → 128+ pass. Commit:**
...
- [x] **Step 1: Viết test fail** — thay nội dung `tests/test_registry.py`:
...
- [x] **Step 3: Tạo `app/registry/args.py`:**
...
- [x] **Step 4: Viết lại `app/registry/registry.py`** (giữ `TOOL_NAMES` + `load_skill` concat schema.md; xóa `DISPATCHABLE`/`is_registered`/`render_tool_catalog`):
...
- [x] **Step 5: Cập nhật 2 importer của `is_registered`:**
...
- [x] **Step 6: Run → pass** `pytest tests/test_registry.py tests/test_session_manager.py tests/test_dispatcher.py -v`.
- [x] **Step 7: Full suite → pass. Commit:**
...
- [x] **Step 1: Thêm vào class `FakeLLM`** (giữ `complete`/`scripted`/`by_role` nguyên — memory compact còn dùng):
...
- [x] **Step 2: Full suite → vẫn pass (chỉ thêm method). Commit:**
...
- [x] **Step 1: Migrate test** — trong `test_tool_data_validator.py`: thay mọi LLM giả trả JSON-string bằng `FakeLLM(structured=[{...}])` từ conftest. Pattern:
...
- [x] **Step 2: Run → fail** (execute còn gọi `llm.complete`).
- [x] **Step 3: Viết lại `execute` trong `data_validator/__init__.py`:**
...
- [x] **Step 4: Run → pass. Full suite → pass. Commit:**
...
- [x] **Step 4: Run → pass. Full suite. Commit:**
...
- [x] **Step 1: Migrate test** — thay `_LLM`/`_SeqLLM`/`_BadJsonLLM` bằng `FakeLLM(structured=[...])`; queue tuần tự = [SqlDraft dict, SemanticCheck dict]:
...
- [x] **Step 2: Run → fail.**
- [x] **Step 3: Viết lại phần LLM của `sql_execute/__init__.py`** — xóa `_coerce_json`, `_parse_sql`, sửa `_PROMPT`/`_CHECK_PROMPT` bỏ chỉ dẫn JSON:
...
- [x] **Step 1: Viết lại `test_session_manager.py`:**
...
- [x] **Step 2: Run → fail.**
- [x] **Step 3: Viết lại `app/tools/session_manager/__init__.py`:**
...
- [x] **Step 5: Migrate `test_orchestrator.py`:** xóa `ScriptLLM`, dùng `FakeLLM(tool_selects=[...])`. Mapping từng test:
...
- [x] **Step 6: Migrate `test_e2e_happy_path.py`:** xóa `RoutingLLM`; dùng MỘT FakeLLM cho cả SM + tool:
...
- [x] **Step 7: Migrate `test_e2e_memory.py`** cùng pattern Step 6 (đọc file, thay LLM giả decision-JSON bằng tool_selects + structured queue; assertions memory giữ nguyên).
- [x] **Step 8: Run → pass:** `pytest tests/test_session_manager.py tests/test_orchestrator.py tests/test_e2e_happy_path.py tests/test_e2e_memory.py -v`
- [x] **Step 9: Full suite → pass. Commit:**
...
- [x] **Step 1: SM skill.md** — giữ nguyên Input contract + toàn bộ 2 khối rule "resolved_require"/"XÁC ĐỊNH CHỦ THỂ THEO NGỮ CẢNH" (chỉ đổi chữ "điền `resolved_require`" thành "điền tham số `resolved_require`/`require` của tool"); thay các mục còn lại:
...
```
- [x] **Step 2: 3 tool skill.md** — xóa mục `## Output schema` (sql_execute:~56, data_validator:~54, answer_composer:~26) và mọi câu "trả về JSON"; thay bằng 1 dòng: `## Output\nOutput trả qua structured channel (function calling) — tập trung vào đúng nghiệp vụ ở trên.` Giữ nguyên toàn bộ rule nghiệp vụ + few-shot SQL.
- [x] **Step 3: Full suite → pass** (skill.md là data; test SM đã stub load_skill).
- [x] **Step 4: Smoke nhanh skill mới với LLM thật** (không vào suite): chạy script Bash gọi `analyze()` với llm thật cho 2 câu: "chào bạn" (mong finish) và "sản phẩm nào đang ế" (mong sql_execute + require). In decision, che key.
- [x] **Step 5: Commit:**
...
- [x] **Step 2: Full suite:** `pytest -q` → toàn bộ pass (mục tiêu ≥ 128).
- [x] **Step 3: Smoke E2E thật:** chạy API local (hoặc script run_session với llm thật + executor thật DATABASE_URL_RO), hỏi "liệt kê 5 khách hàng mua nhiều nhất" → answer có dữ liệu + "Gợi ý:"; hỏi "sản phẩm nào đang ế" → SQL có LEFT JOIN. Ghi elapsed các LLM call từ log so với trước.
- [x] **Step 4: Commit cuối + cập nhật plan checkboxes:**
