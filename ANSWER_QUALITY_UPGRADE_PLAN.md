...
| **Đã chốt** | **A** |
| **Ảnh hưởng** | A → chỉ sửa plan Phase 2–3. B/C → thêm node/edge, timeout SQL, test `TEST_AI`. |
...
| | |
|---|---|
| **Câu hỏi** | Tiêu chí done có chạy `TEST_AI/run_test.py` / pytest không? |
| **Đề xuất** | Thêm ≥3 case vào `TEST_AI` (empty SQL, domain reject, chart_fail) + unit test `check_answer_quality` với profiles. |
...
**Done khi:** unit test + 1 manual case trong TEST_AI.
